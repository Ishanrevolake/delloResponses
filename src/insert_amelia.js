...
async function addAmeliaSite() {
  const { data, error } = await supabase
    .from('websites')
    .insert([
      { name: 'Amelia Mabel', domain: 'https://ameliamabel.dellodigital.com/' }
    ])
    .select();
  ...
  if (error) {
    if (error.code === '23505') { // unique constraint violation
         ...
         const { data: existingData } = await supabase.from('websites').select('*').eq('domain', 'https://ameliamabel.dellodigital.com/');
         if (existingData && existingData.length > 0) {
             console.log('WEBSITE_ID:', existingData[0].id);
         }
    } else {
    ...
    console.log('WEBSITE_ID:', data[0].id);
  }
}
